...
!(async () => {
  ...
  console.log(`活动地址:https://jinggengjcq-isv.isvjcloud.com/fronth5/#/pages/unitedCardNew20220312/unitedCardNew20220212?actId=628b293b355c42e39d5fea6f583e_20220212`)
  ...
  try {
    ...
    for(o of $.openList){
      ...
      if(o.open != true && o.openCardUrl){
        ...
        if($.openCard == true){
          let newVar = await joinShop();
            for (let c=0;c<3;c++) {
              if (newVar.busiCode !=0 ){
                await $.wait(3000)
                newVar = await joinShop();
    ...
    if(guaopencard_addSku+"" == "true"){
      console.log('如需加购 请自己写');
    }else{
      console.log('如需加购请设置环境变量[guaopencard_addSku]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw]为"3" 3为次数');
