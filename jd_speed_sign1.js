...
================Loon==============
[Script]
cron "21 6,14 * * *" script-path=jd_speed_sign.js,tag=京东极速版
*/
...
!(async () => {
  ...
  // var ii = randomNum(0,60)



  for (let i = Math.floor(cookiesArr.length/4); i < Math.ceil(cookiesArr.length/2); i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
